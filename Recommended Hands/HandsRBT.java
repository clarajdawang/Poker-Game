...
public class HandsRBT {
    ...
    {
        //left rotation only possible if there is right child
        if (thisNode.right == null || thisNode == null){
            return;
        }

        HandsRBTNode x = thisNode.right; //x is the right child
        thisNode.right = x.left; //move x left subtree to be right subtree of thisNode
        
        //update x parent reference
        if (x.left != null) {
            x.left.parent = thisNode;
        }

        x.parent = thisNode.parent;

        //if thisNode was the root, update root reference to x
        if (thisNode.parent == null) {
            root = x;
        } 
        
        //otherwise, update parents left or right child to pont to x
        else if (thisNode == thisNode.parent.left) {
        thisNode.parent.left = x;
        } 
        
        else {
        thisNode.parent.right = x;
        }
        
        //move thisNode under x as the left child
    ...
    {
        //rotation only possible if there is a left child
        if (thisNode.left == null || thisNode == null){

             return;
        }

        HandsRBTNode x = thisNode.left; //x is the left child of thisNode
        thisNode.left = x.right; //Move x right subtree to be the left subtree of thisNode

        //if xs right subtree is not null, update its parent reference
        if (x.right != null) {
            x.right.parent = thisNode;
        }
        //update x parent to be thisNodes parent
        x.parent = thisNode.parent;
        ...
        //otherwise, update parents left or right child to point to x
        else if (thisNode == thisNode.parent.left) {
            thisNode.parent.left = x;
        } 
        ...
        //move thisNode under x as right child
        x.right = thisNode;
        thisNode.parent = x;
        }    
    ...
    public void insert(Hands thisHand)
    {
        ...
        //check if hand already exists in tree, if yes, exit without inserting
        if (findNode(thisHand) != null){
            return;
        }

        HandsRBTNode p = null; //parent of current node
        HandsRBTNode x = root; //starting traversal from root

        //find correct place to insert
        while (x != null){
            p = x; //keep track of parent node

            //compare current hand (thisHand) with current x. 
            if (thisHand.isMyHandLarger(x.myHand)){
                x = x.right; //move right if the hand is larger
            }
            else{
                x = x.left; //move left if the hand is smaller
            }
        }
        // Else go to step 2
        // Step 2: Insert the new node. If it is the root, colour it BLACK. Else colour it RED

        //mkae a new node for thisHand and initialize its parent to p (parent tracked)
        HandsRBTNode newNode = new HandsRBTNode(thisHand);
        newNode.parent = p;

        //if the parent is null then new node becomes root as the tree is empty
        if (p == null){
            root = newNode;
            newNode.colour = BLACK; //if this is the root, set to black
            return;
        } 

        //set new node as left or right child if parent is not null
        else if (thisHand.isMyHandLarger(p.myHand)){
            p.right = newNode;
        }
        else{
            p.left = newNode;
    ...
    // Recall that there are 3 cases:
     // Case 1. Red Uncle. (Hare you have to check if the red violation moves up the tree; if so recurse.
     ...
     // Case 3. Black Uncle & Innergrandchild   
    {
        ...
        if (thisNode == root) {
            thisNode.colour = BLACK; //root needs to be black (property)
            return;
        }

        HandsRBTNode parent = thisNode.parent; //get parent, if its black then there is no violation
        if (parent.colour == BLACK) return;

        HandsRBTNode grandparent = parent.parent;
        if (grandparent == null) {
            parent.colour = BLACK; //if parent is root, node black
            return;
        }
        ...
        if (uncle != null && uncle.colour == RED) {
            ...
            fixRedViolation(grandparent);//if there are any violations due to recolouring, call fixRed
        } else {
            //Case 2: black uncle with outer grandchild
            //left rotation to make node inner child
            if (parent == grandparent.left) {
                if (thisNode == parent.right) {
                    rotateLeft(parent);
                    parent = thisNode; //now parent after rotation
                }
                rotateRight(grandparent);
                parent.colour = BLACK;
                grandparent.colour = RED;

                //Case 2: black uncle with inner grandchild
            } else {
    ...
    {
        for (int i = 0; i < 5; i++) {
            Card card = consumedHand.getCard(i);
            deleteHandsWithCard(card);
        }
    }
    ...
    private void deleteHandsWithCard(Card thisCard)
    {
        ...
        collectHandsWithCard(root, thisCard, handsToDelete);
        for (Hands hand : handsToDelete) {
            delete(hand);
        }
    }

    private void collectHandsWithCard(HandsRBTNode node, Card thisCard, Vector<Hands> result) {
        if (node == null) {
            return;
        }
        if (node.myHand.hasCard(thisCard)) {
            result.add(node.myHand);
        }
        collectHandsWithCard(node.left, thisCard, result);
        collectHandsWithCard(node.right, thisCard, result);
    }
    ...
    {// Setup
        ...
        HandsRBT testRBT = new HandsRBT();
        Hands myHandsArray[] = new Hands[5];        
        myHandsArray[0] = new Hands(new Card(5, 'C'), new Card(2, 'H'), new Card(5, 'S'), new Card(2, 'D'), new Card(5, 'H')); //full house
        myHandsArray[1] = new Hands(new Card(5, 'S'), new Card(5, 'D'), new Card(5, 'C'), new Card(3, 'H'), new Card(5, 'H')); //four of a kind (5)
        myHandsArray[2] = new Hands(new Card(2, 'H'), new Card(3, 'C'), new Card(4, 'D'), new Card(5, 'H'), new Card(2, 'S')); //straight
        myHandsArray[3] = new Hands(new Card(6, 'H'), new Card(7, 'D'), new Card(7, 'C'), new Card(7, 'H'), new Card(7, 'S')); //four of a kind (7)
        myHandsArray[4] = new Hands(new Card(9, 'H'), new Card(6, 'D'), new Card(6, 'S'), new Card(6, 'C'), new Card(6, 'S')); //four of a kind (6)
        testRBT.insert(myHandsArray[0]);
        testRBT.insert(myHandsArray[1]);
        testRBT.insert(myHandsArray[2]);
        testRBT.insert(myHandsArray[3]);
    ...
    private static void testCustomInsertCase5()
    {
        ...
        HandsRBT testRBT = new HandsRBT();
        Hands myHandsArray[] = new Hands[5];        
        myHandsArray[0] = new Hands(new Card(5, 'C'), new Card(2, 'D'), new Card(5, 'S'), new Card(5, 'H'), new Card(2, 'H')); //full house
        myHandsArray[1] = new Hands(new Card(6, 'D'), new Card(6, 'S'), new Card(3, 'H'), new Card(6, 'C'), new Card(6, 'H')); //royal flush
        myHandsArray[2] = new Hands(new Card(2, 'C'), new Card(3, 'D'), new Card(4, 'H'), new Card(5, 'S'), new Card(5, 'H')); //straight
        myHandsArray[3] = new Hands(new Card(7, 'D'), new Card(7, 'S'), new Card(7, 'H'), new Card(6, 'H'), new Card(7, 'C')); //four of a kind
        myHandsArray[4] = new Hands(new Card(10, 'S'), new Card(11, 'S'), new Card(12, 'S'), new Card(13, 'S'), new Card(14, 'S')); //royal flush
        testRBT.insert(myHandsArray[0]);
        testRBT.insert(myHandsArray[1]);
        testRBT.insert(myHandsArray[2]);
        testRBT.insert(myHandsArray[3]);
    ...
    private static void testCustomDeleteHandsWithCard()
        ...
        HandsRBT testRBT = new HandsRBT();
        Hands myHandsArray[] = new Hands[5];        
        myHandsArray[0] = new Hands(new Card(7, 'H'), new Card(2, 'D'), new Card(7, 'C'), new Card(7, 'S'), new Card(4, 'H'));
        myHandsArray[1] = new Hands(new Card(7, 'D'), new Card(7, 'H'), new Card(2, 'S'), new Card(7, 'C'), new Card(7, 'S'));
        myHandsArray[2] = new Hands(new Card(2, 'H'), new Card(4, 'D'), new Card(5, 'C'), new Card(7, 'H'), new Card(6, 'S'));
        ...
        testRBT.deleteHandsWithCard(new Card(7, 'H'));
